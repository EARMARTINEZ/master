...
const Comenta = () => {
  ...
  const onReset = () => {
    ...
    const { RefePendings, doUpdatePendingsComment } = useTasks(); 
    ...
    let ComentaObj= [];
    RefePendings?.map((Comments, index) => {    
    ...
    const handleSubmit = (values) => {    
      ...
      //console.log(values)
        ...
        doUpdatePendingsComment(undefined, value,'Showroom', values);
        onReset();
        setSubmitting(false);
        setValue('');      
        setComments([
...
export function FormPending() { 
  ...
  const { Referencia, RefePendings } = useTasks();
  ...
  let Pendingslength=[]; 
  RefePendings?.map((Pending, index) => {
